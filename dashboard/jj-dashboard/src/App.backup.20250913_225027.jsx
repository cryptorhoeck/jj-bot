...
export default function UltimateJJDashboard() {
  ...
  const handleToggleSimulator = async () => {
    try {
      const endpoint = isSimulatorRunning ? '/api/simulator/stop' : 'http://127.0.0.1:8000/api/simulator/start';
  ...
  const runBacktest = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8000/api/backtest/run', {
        method: 'POST',
